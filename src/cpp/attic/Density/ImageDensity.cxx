...
#include "ConvolutedImageDensity.h"
...
template<class TF>
class ImageDensityIterator : public DensityIterator<TF> {
public:
    ...
    virtual void display( Displayer &ds ) const override {
        DS_OBJECT( ImageDensityIterator, cl, this->x0, this->x1, i );
    }
    ...
    const ImageDensity<TF> *cl;
    PI i;
};

#define DTP template<class TF>
#define UTP ImageDensity<TF>

DTP UTP::ImageDensity( const Vec<TF> &xs, const Vec<TF> &ys ) : xs( xs ), ys( ys ) {
...
DTP RcPtr<Density<TF>> UTP::convoluted( RcPtr<Convolution<TF>> convolution ) const {
    return new ConvolutedImageDensity<TF>( xs, ys, convolution );
}

DTP RcPtr<DensityIterator<TF>> UTP::iterator() const {
    auto *res = new ImageDensityIterator<TF>;
    ASSERT( xs.size() >= 2 );
    res->x0 = xs[ 0 ];
    res->x1 = xs[ 1 ];
    res->cl = this;
